...
class FFCResNetBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if use_film:
            self.diffusion_projection = nn.Linear(512, in_channels * 2)
        else:
            self.diffusion_projection = nn.Linear(512, in_channels)

        if use_film:
            self.film = FiLM()

    def forward(self, x, t):
        emb_out = self.diffusion_projection(t)
        while len(emb_out.shape) < len(x.shape):
            emb_out = emb_out[..., None]
        if hasattr(self, "film"):
            x = self.film(x, emb_out)
        else:
            x = x + emb_out
        out = self.ffc1(x)
        out = self.ffc2(out)
