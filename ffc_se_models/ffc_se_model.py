...
class UNetBaseBlock(torch.nn.Module):
    ...
    def forward(self, x, t):
        for i, mod in enumerate(self.model):
            if i == 2:
                for mod2 in mod:
                    if isinstance(mod2, FFCResNetBlock):
                        x = mod2(x, t)
                    else:
                        x = mod2(x)
            else:
...
class _InnerWrapper(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        x = self._pad_to_divisible(
            x,
            modulo=2**self.config.scale_factor,
            mode=self.config.padding_type,
        )
        for i, mod in enumerate(self.net):
            if i == 1:
                for mod2 in mod:
                    if isinstance(mod2, UNetBaseBlock):
                        x = mod2(x, t)
                    else:
                        x = mod2(x)
                ...
                x = mod(x)
        out = x[..., :dim_2, :dim_1].permute(0, 2, 3, 1)
...
class FFCSE(torch.nn.Module):
    def __init__(
        ...
        model = nn.ModuleList(
            [
                FFCResNetBlock(
                    ...
                    use_film=model_config.use_film,
                )
        ...
        model = _InnerWrapper(model, model_config)
    ...
    def forward(self, x, t=torch.tensor(100)):
        # for i, mod in enumerate(self.model):
        # if i == 1:
